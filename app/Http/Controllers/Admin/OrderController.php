...
class OrderController extends Controller
{
    ...
    public function packing($id)
    {
        ...
        if($order->order_type == 'default_type')
        {
            return view('admin-views.order.packing-details', compact('shipping_address','order', 'linked_orders', 'delivery_men', 'total_delivered', 'company_name', 'company_web_logo', 'physical_product'));
        }else{
            return view('admin-views.order.packing-details', compact('shipping_address','order', 'linked_orders', 'delivery_men', 'total_delivered', 'company_name', 'company_web_logo', 'physical_product'));
        }
    }
...
public function generateAllInvoices(Request $request)
    {
        $selectedOrderIds = $request->input('selected_records', []);
        ...
        $orders = Order::whereIn('id', $selectedOrderIds)
            ->with('seller', 'shipping', 'details', 'customer')
            ->get();
            // dd($orders);
        $view = view('admin-views.order.all-invoice', compact('orders', 'company_name', 'company_web_logo'))->render();
        $pdf->WriteHTML($view);
 ...
 public function downloadSelectedPackingPDF(Request $request)
{
...
//   dd($orders);
    ...
    $pdfContent = view('admin-views.order.all-packing', compact('orders', 'company_name', 'company_web_logo'))->render();
    // dd($pdfContent);
    ...
    return response($pdfString)
        ...
        ->header('Content-Disposition', 'attachment; filename="selected_packing.pdf"');
}
...
protected function generateInvoicePdf($order, $company_name, $company_web_logo)
{
    // Retrieve and format data for the invoice
    ...
    $html = '
        ...
        <body>
            <table style="width: 100%;">
                <tr>
                    <td>
                        <img src="' . asset("storage/app/public/company/$company_web_logo") . '" height="50" alt="">
                    </td>
            ...
            <h1 style="text-align:center">Invoice for Order #' . $orderNumber . '</h1>
    ...
    {        
        ...
        return view('admin-views.order.barcode', compact('order', 'limit'));
    }
    ...
    {
        $order_ids_string = $request->ord_id;
        
        if($order_ids_string == null)
        {
            $search = $request['search'];
        $filter = $request['filter'];
        $from = $request['from'];
      ...
      return (new FastExcel($storage))->download('Order_All_details.xlsx');
        ...
        {
            $orderList[] = Order::with('customer')->where('id', $order)->first();
        }
        ...
        return (new FastExcel($storage))->download('Order_details.xlsx');
    }
    }
   
    public function export_packing_data(Request $request, $status)
    {
        $order_ids_string = $request->ord_id;
        ...
        foreach ($orders as $item) {
            ...
            $storage[] = [
                ...
                'Seller Name' => isset($item->seller)? $item->seller->f_name. ' '.$item->seller->l_name:'not found',
                'Seller Phone'  => isset($item->seller)? $item->seller->phone:'not found',
                'Customer Address' => isset($item->shippingAddress)? $item->shippingAddress->address:'not found',
                'Division' => isset($item->shippingAddress)? $item->shippingAddress->division:'not found',
                'City' => isset($item->shippingAddress)? $item->shippingAddress->city:'not found',
                'Thana' => isset($item->shippingAddress)? $item->shippingAddress->thana:'not found',
                'Zip' => isset($item->shippingAddress)? $item->shippingAddress->zip:'not found',
                'Order Amount' => Helpers::currency_converter($order_amount)
            ];
        }
