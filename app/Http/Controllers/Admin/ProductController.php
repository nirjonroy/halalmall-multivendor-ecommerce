...
class ProductController extends BaseController
{
    ...
    {   
        ...
        if ($request->ajax()) {
            ...
            //         $product_img[] = $img;
            //     }
            //     $p->images = json_encode($product_img);
            // }
            ...
            //         $product_images[] = ImageManager::upload('product/', 'jpg', $img);
            ...
            //     if (!empty($image_data)) {
            //         $p->images = json_encode($image_data);
            //     }
            // }
            
            $p->thumbnail = $request->product_thumb_image_name[0];
        ...
        //   if($request->variation_code != null) {          
              ...
              foreach($request->products_variation_image_name as $key => $pro_variation_img) {
               ...
               if (!empty($variation_image_value_name)) {
                ...
                foreach ($variation_image_value_name as $key => $imageName) {
                    ...
                    $p->variation_images()->createMany($variationImages);
                }
    ...
    foreach($all_cats as $key => $value) {
        ...
        $digital_product_setting = BusinessSetting::where('type', 'digital_product')->first()->value;
        $weights = ProductWeight::all();
        return view('admin-views.product.edit', compact('categories','sub_cat_test','sub_sub_cat_test','sub_cat','sub_sub_cat', 'br', 'product', 'product_category','brand_setting','digital_product_setting', 'weights'));
    }
