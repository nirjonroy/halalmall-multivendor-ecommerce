...
class DealController extends Controller
{
    ...
    public function flash_submit(Request $request)
    {
        ...
        $flash_deal_id = DB::table('flash_deals')->insertGetId([
            ...
            'minimum_discount_amount' => $request['minimum_discount_amount'],
            'status' => 0,
    ...
    public function shipping_index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $free_shipping = FlashDeal::where('deal_type', 'free_shipping');
        }
        $free_shipping = $free_shipping->latest()->paginate(Helpers::pagination_limit())->appends($query_param);
        return view('admin-views.deal.shipping-index', compact('free_shipping', 'search'));
    }
    ...
    public function add_product($deal_id)
    {
        ...
        $flash_deal_product = FlashDealProduct::where('flash_deal_id', $deal_id)->with('product')->get();
        ...
        return view('admin-views.deal.add-product', compact('deal', 'products','flash_deal_products','flash_deal_product'));
    }
    ...
    public function fshipping_product($deal_id)
    {
        ...
        return view('admin-views.deal.fshipping_product', compact('deal', 'products','flash_deal_products'));
    }

    public function add_product_submit(Request $request, $deal_id)
    {
        $user_name = auth('admin')->user()->f_name;
        $user_id = auth('admin')->user()->id;
        $deal_type = $request->deal_type;
    ...
    {
        $delete_campaign = FlashDeal::find($id);
        $flashProduct = $delete_campaign->products;
        if(count($flashProduct) > 0)
        {
            foreach($flashProduct as $product)
            {
                $delProduct = FlashDealProduct::where('product_id', $product->product_id);
        ...
        // if($delete_campaign->products->count() > 0)
        // {
        //     foreach($delete_campaign->products() as $product)
        //     {
        //         dd($product);
        //     }
        // } else {
    ...
    public function discount_update(Request $request)
    {
        ...
        FlashDealProduct::where('product_id', $request->product_id)->update([
            'added_status' => 1     
        ]);
