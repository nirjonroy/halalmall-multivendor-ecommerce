...
class RegisterController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function ($r) use ($request) {
            ...
            $shop->side_banner = ImageManager::upload('shop/banner/', 'png', $request->file('side_banner'));
            $shop->save();
          
            $all_admin = Admin::all();
          
            foreach($all_admin as $admin) {
            	$new_chat = new Chatting();
                $new_chat->admin_id = $admin->id;
                $new_chat->seller_id = $seller->id;
                $new_chat->shop_id = $shop->id;
                $new_chat->message = 'Hi, I am new comer..!!';
                $new_chat->sent_by_seller = 1;
                $new_chat->seen_by_seller = 1;
                $new_chat->created_at = now();     
                $new_chat->save();         
            }     
