...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $categories = Category::where(['position'=> 0])->priority()->take(11)->where('is_popular', 1)->get();
		...
		$dealsr = DealOfTheDay::join('products', 'products.id', '=', 'deal_of_the_days.product_id')->select('deal_of_the_days.*', 'products.*')->where('deal_of_the_days.status', 1)->get();
        
        $deals = DealOfTheDay::with('product')->get();    
        
        
        return view('web-views.home',
                    compact('featured_products', 'topRated','deals', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories', 'brand_setting')
                );
    }
    ...
    public function feature_deals()
    {
        ...
        if (isset($deal)) {
            return view('web-views.deals2', compact('deal', 'discountPrice'));
        }
        Toastr::warning(translate('not_found'));
        return back();
    }
    ...
    public function free_shipping()
    {
        ...
        $free_shipping = FlashDeal::with(['products.product.reviews', 'products.product' => function($query){
                $query->active();
            }])->where('deal_type', 'free_shipping')
            ->get();
        ...
        if (isset($free_shipping)) {
            return view('web-views.deals3', compact('free_shipping', 'discountPrice'));
        }
        Toastr::warning(translate('not_found'));
        return back();
    }
...
public function shop_view_cat(Request $request, $id, $category_id) {
    
    // dd($products);
    // Pass the $products variable to your view for rendering
    // return view('your_view_name', ['products' => $products]);
        ...
        // $popularCategories = PopularCategory::with('category')->where('seller_id', $id)->get();
        // dd($popularCategories);
        $seller = Seller::find($id);
        $helper = new \App\CPU\Helpers();
        $popularCats = $helper->popularCategories();
        $popularProducts = [];
        
        foreach ($popularCats as $pCats) {
            $poProducts = Product::where('category_id', $pCats->category_id)
                                ->active()
                            ->where('user_id', $id)
                            ->get();
    ...
    // dd($popularCats);
        ...
        $all_product = Product::with('shop')->where(['added_by' => 'seller'])        
        ...
        $cat_wise_pro = Category::join('products', 'products.category_ids', 'categories.id')
                        ->get();    


    $cat_products = Product::whereIn('id', $product_ids)->get();


    $categoryIds = $cat_products->pluck('category_ids.*.id')->unique();
            // dd($categoryIds);
    $categoryProducts = [];

    
    foreach ($cat_wise_pro as $categoryId) {
        
        $cat_products_by_category = Product::whereJsonContains('category_ids', [['id' => strval($categoryId)]])->get();
        $categoryProducts[$categoryId] = $cat_products_by_category;
        
    }

    
         
    $cat_wise_prod = Product::with('category')
    ->has('category')
...
foreach($categories as $pcats){
    $cat_pro = Product::with('shop', 'category')
                ->has('category')
            ->where('user_id', $id)->where('category_id', $pcats->id)
            ->get();
            $catwise_prod[$pcats->id] = $cat_pro;
            // dd($catwise_prod);
        ...
        // return view('web-views.cat_wise_shop', compact('productss','single_category','products', 'shop', 'categories', 'shop_cat', 'shop_cat4', 'catwise_prod'))
        //     ->with('seller_id', $id)
        //     ->with('total_review', $total_review)
        //     ->with('avg_rating', $avg_rating)
        //     ->with('total_order', $total_order);
    
    return view('web-views.cat_wise_shop', compact('productss','single_category','products', 'shop', 'categories', 'shop_cat', 'shop_cat2', 'shop_cat3', 'shop_cat4', 'cat_wise_pro', 'cat_wise_prod', 'catwise_prod', 'popularCats',
                'poProducts',
                'popularProducts'))
            ->with('seller_id', $id)
            ->with('total_review', $total_review)
    ...
    // return view('web-views.cat_wise_shop', compact('productss','single_category'));
}
...
foreach($categories as $pcats){
    $cat_pro = Product::with('shop', 'category')
        ...
        //products search
        if ($request->product_name) {
            $products_search = Product::active()
                ->when($id == 0, function ($query) {
                    return $query->where(['added_by' => 'admin']);
                })
                ->when($id != 0, function ($query) use ($id) {
        ...
        $single_category = Category::find($request->category_id);
    
         $productss = Product::with('shop', 'category')
        ->where('user_id', $id)
        ...
        return view('web-views.shop-page', compact('single_category','productss','products','products_search', 'shop', 'categories', 'shop_cat', 'shop_cat2', 'shop_cat3', 'shop_cat4', 'cat_wise_pro', 'cat_wise_prod', 'catwise_prod', 'popularCats',
                'poProducts',
                'popularProducts'))
            ->with('seller_id', $id)
            ->with('total_review', $total_review)
