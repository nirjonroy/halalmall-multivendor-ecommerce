...
<head>
    ...
    @php($seller_pixel_analytices_user_code =\App\CPU\Helpers::get_business_settings('seller_pixel_analytics'))
    @if($pixel_analytices_user_code)
        <!-- Facebook Pixel Code -->
            {!! $pixel_analytices_user_code !!}
        <!-- End Facebook Pixel Code -->
    @endif
    
    @if($seller_pixel_analytices_user_code)
        <!-- Facebook Pixel Code -->
            {!! $seller_pixel_analytices_user_code !!}
        <!-- End Facebook Pixel Code -->
    @endif
</head>
<!-- Body-->
        ...
        if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.variant_prce').html(data.price);
                    $('#variant_prcee').html(data.price);
                    // $('#add-to-cart-form .variant_prce').html(data.price);
