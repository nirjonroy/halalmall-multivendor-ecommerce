    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\App\CPU\translate('Chatting_List')}} - Chat with {{$type}}
            </h2>
        </div>
        <!-- End Page Title -->
        ...
        <div class="row">
                                ...
                                @foreach($chattings_user as $key => $chatting)
                                    <div class="list_filter">
                                        <div class="chat_list p-3 d-flex gap-2 user_{{$chatting->user_id ? $chatting->user_id : ($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->admin_id) }} seller-list @if ($key == 0) active @endif"
                                             id="{{$chatting->user_id ? $chatting->user_id : ($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->admin_id) }}" data-name="{{$chatting->id ? $chatting->name : $chatting->f_name.' '.$chatting->l_name }}" data-phone="{{ $chatting->phone }}">
                                                ...
                                                <div class="chat_ib media-body">
                                                   ...
                                                   if($type == 'customer') {
                                                      ...
                                                      <h5 class="mb-1 seller @if($chatting->seen_by_seller)active-text @endif"
                                                        id="{{$chatting->user_id ? $chatting->user_id :($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->id) }}" data-name="{{$chatting->user_id ? $chatting->f_name : $chatting->f_name.' '.$chatting->l_name }}" data-phone="{{ $chatting->phone }}">
                                                        {{$chatting->user_id ? $chatting->f_name.' '.$chatting->l_name : $chatting->f_name.' '.$chatting->l_name }}
                                                        <br><span class="mt-2 font-weight-normal text-muted" id="{{$chatting->user_id ? $chatting->user_id : ($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->id) }}" data-name="{{$chatting->user_id ? $chatting->name : $chatting->f_name.' '.$chatting->l_name }}" data-phone="{{ $chatting->phone }}">{{ $chatting->phone }}</span>
                                                    </h5>
                                                       ...
                                                       ?>
                                                       <h5 class="mb-1 seller @if($chatting->seen_by_seller)active-text @endif"
                                                        id="{{$chatting->id ? $chatting->id :($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->id) }}" data-name="{{$chatting->id ? $chatting->name : $chatting->f_name.' '.$chatting->l_name }}" data-phone="{{ $chatting->phone }}">
                                                        {{$chatting->id ? $chatting->name : $chatting->f_name.' '.$chatting->l_name }}
                                                        <br><span class="mt-2 font-weight-normal text-muted" id="{{$chatting->id ? $chatting->id : ($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->id) }}" data-name="{{$chatting->id ? $chatting->name : $chatting->f_name.' '.$chatting->l_name }}" data-phone="{{ $chatting->phone }}">{{ $chatting->phone }}</span>
                                                    </h5>
                                                    <?php
                                                   }
                                                   ?> 
                                            ...
                                            <form class="mt-4" id="myForm">
                                                @csrf
                                                    <input type="text" id="hidden_value" hidden
                                                           value="{{$chatting->user_id ? $chatting->user_id : ($chatting->delivery_man_id ? $chatting->delivery_man_id : $chatting->admin_id) }}" name="hidden_value">
                                                ...
                                                <div class="mt-3 d-flex justify-content-end">
                                                    <input type="file" id="chat_image" name="chat_image" style="width: 50%;margin-top: 10px;" class="form-control">
            ...
            @else
                <div class="offset-md-1 col-md-10 d-flex justify-content-center align-items-center">
                    <p>{{\App\CPU\translate('No conversation founds')}}</p>
                </div>
            @endif
...
@push('script')
    <script>
                ...
                success: function (data) {
                    ...
                    if (data.length != 0) {
                        data.map((element, index) => {
                            ...
                            if (element.sent_by_seller && element.user_id == null) {
                            ...
                            <div class="outgoing_msg" id="outgoing_msg">
                              <div class='send_msg'>
                               <img src="{{asset('public/seller_to_admin_image/${element.image}')}}" onclick="window.open(this.src, '_blank');" class="rounded-circle border"
                         height="200" width="200" style="border-radius: 15% !important;" alt="">
                            ...
                            else if(element.sent_by_seller && element.admin_id == null) 
                            {
                                if (element.message == '' || element.message == null) {
                                    $(".msg_history").prepend(`
                                      <div class="outgoing_msg" id="outgoing_msg">
                                        <div class='sent_msg'>
                                        <img src="{{asset('public/seller_to_customer_image/${element.image}')}}" onclick="window.open(this.src, '_blank');" class="rounded-circle border"
                         height="200" width="200" style="border-radius: 15% !important;" alt="">
                                          <span class='time_date'> ${time}    |    ${date}</span>
                                        </div>
                                      </div>`
                            ...
                            else if(element.sent_by_customer == 1 && element.admin_id == null)
                            {
                               if (element.message == '' || element.message == null) {
                                    $(".msg_history").prepend(`
                                      <div class="incoming_msg" id="outgoing_msg">
                                        <div class='sent_msg'>
                                        <img src="{{asset('public/customer_to_seller_image/${element.image}')}}" onclick="window.open(this.src, '_blank');" class="rounded-circle border"
                         height="200" width="200" style="border-radius: 15% !important;" alt="">
                         <br/>
                                          <span class='time_date'> ${time}    |    ${date}</span>
                                        </div>
                                ...
                                if (element.message == '' || element.message == null) {
                                    $(".msg_history").prepend(`
                                      <div class="incoming_msg" id="outgoing_msg">
                                        <div class='sent_msg'>
                                        <img src="{{asset('public/admin_to_seller_image/${element.image}')}}" onclick="window.open(this.src, '_blank');" class="rounded-circle border"
        ...
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (respons) {
                        ...
                        $(".msg_history").prepend(`
                                  <div class="outgoing_msg" id="outgoing_msg">
                                    <div class='sent_msg'>
                                    <img src="${respons.fileName}" class="rounded-circle border"
                         height="200" width="200" alt="">
            ...
            var pusher = new Pusher('ebe1a63232843e8f2752', {
                            cluster: 'ap2'
                        });
