            ...
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100" cellspacing="0">
                        ...
                        <tbody>
                        @foreach($flash_deal_product as $k=>$de_p)
                        
                            <tr>
                                <td>{{$products->firstitem()+$k}}</td>
                                <td><a href="#" target="_blank" class="font-weight-semibold title-color hover-c1">{{$de_p['product']['name']}}</a></td>
                                <td>{{ $de_p->user_name }}</td>
                                ...
                                <input type="hidden" class="product_id" value="{{ $de_p['product']['id'] }}">
                                <input type="hidden" class="minimum_discount_amount" value="{{ $de_p['id'] }}">
                                <td style="width: 15%;">
                                    <input class="discount_value form-control" value="{{ $de_p['product']['discount'] }}" type="number" name="discount">
                                </td>
                                <td>
                                    <?php
                                     $unit_price = $de_p['product']['unit_price'];
                                     $discount = $de_p['product']['discount'];
...
@push('script')
    ...
    <script>
        ...
        $(document).on('blur', '.discount_value', function () {
            ...
            let mdp = Number($('.minimum_discount_amount').val());
            ...
            $.ajax({
                url: "{{route('admin.deal.discount-update')}}",
                method: 'GET',
                data: {
                ...
                success: function (res) {
                    if(res.success)
                    {
                        toastr.success('{{\App\CPU\translate('Discounts updated successfully')}}');
                    }
