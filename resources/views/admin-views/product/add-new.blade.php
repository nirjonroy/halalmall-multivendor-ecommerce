            ...
            <div class="col-md-12">
                <form class="product-form" action="{{ route('admin.product.store') }}" method="POST"
                    ...
                    <div class="card">
                            ...
                            @foreach (json_decode($language) as $lang)
                                <div class="{{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                    ...
                                    <div class="form-group">
                                        <label class="title-color text-danger"
                                            for="{{ $lang }}_name"><strong>{{ \App\CPU\translate('name') }}
                                            ({{ strtoupper($lang) }}) <span class="text-danger">***</span></strong>
                                        </label>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <label for="name" class="title-color text-danger">
                                    <strong>
                                        {{ \App\CPU\translate('Category') }} <span class="text-danger">***</span>
                                    </strong>
                                    
                                    </label>
                                <select class="js-example-basic-multiple form-control" id="category-select" name="category_id"
                                            onclick="getRequest('{{ url('/') }}/admin/product/get-categories-click?id='+this.value,'sub-category-select','no-select')"
                                            required>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="js-example-basic-multiple form-control" name="sub_category_id"
                                    ...
                                    onclick="getRequest('{{ url('/') }}/admin/product/get-sub-categories-click?id='+this.value,'sub-sub-category-select','no2-select')">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-8 form-group">
                                    ...
                                    <input id="products_image_name" type="hidden" name="products_image_name[]">
                                </div>

                                <div class="col-md-4 form-group">
                                    <div class="mb-2">
                                        <label for="name" class="title-color text-capitalize">{{ \App\CPU\translate('Upload thumbnails') }}</label>
                                        <span class="text-info">* ( {{ \App\CPU\translate('ratio') }} 1:1 )</span>
                                    </div>
                                    <div>
                                        <div class="row" id="thumbnail"></div>
                                    </div>
                                    <input id="product_thumb_image_name" type="hidden"  name="product_thumb_image_name[]">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <label for="name" class="title-color text-danger">
                                   <strong>{{ \App\CPU\translate('product_type') }} <span>***</span></strong> 
                                    </label>
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <label for="name" class="title-color text-danger">
                                   <strong>{{ \App\CPU\translate('Brand') }} <span>***</span></strong> 
                                </label>
                                <select
                            ...
                            <div class="row align-items-end">
                                <div class="col-md-6">
                                    ...
                                    <select
                                        ...
                                        @foreach (\App\Model\Color::orderBy('id', 'desc')->get() as $key => $color)
                        ...
                        <div class="card-body">
                            <div class="row align-items-end">
                                <div class="col-md-6 form-group">
                                    <label class="title-color text-danger">
                                        <strong>{{ \App\CPU\translate('Unit price') }} <span>***</span></strong>
                                ...
                                <div class="col-md-6 form-group">
                                    <label class="title-color text-danger">
                                        <strong>{{ \App\CPU\translate('Purchase price') }} <span>***</span></strong>
                                ...
                                <div class="col-md-3 form-group physical_product_show d-none" id="shipping_cost_multy">
                                    <div>
                                        <label
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4 mb-4">
                                    <label class="title-color text-danger">
                                        <strong>
                                            Choose Weight (for shipping charge dynamic) <span>***</span>
                                        </strong>
                                    </label>
    ...
    <script>
        $(function() {
            ...
            $("#coba").spartanMultiImagePicker({
            	...
            	directUpload : {
            		...
            		success : function(data, textStatus, jqXHR){
            		    product_array.push(textStatus);
            		    $('#products_image_name').val(product_array);
            		},
            ...
            $("#thumbnail").spartanMultiImagePicker({
                fieldName: 'product_thumb_image_name', // this configuration will send your images named "fileUpload" to the server
            	directUpload : {
            		status: true,
            		loaderIcon: `<div class="spinner-border text-primary"></div>`, // spinner class from bootstrap
            		url: '{{ route('admin.product.store_thumb_image')}}',
            		additionalParam : {
            			name : 'My Name'
            		},
            		success : function(data, textStatus, jqXHR){
            		    $('#product_thumb_image_name').val(textStatus);
            		},
            		error : function(jqXHR, textStatus, errorThrown){
            ...
            $.get({
                ...
                success: function(data) {
                    if (type == 'select') {   
                        ...
                        if (data.sign == 'sub-cat') {
                            $('#category-select').empty();
                            $('#sub-category-select').empty();
                            $('#sub-sub-category-select').empty();                     
                            $('#'+data.id).empty().append(data.select_tag);
                            $('#'+data.idt).empty().append(data.select_tagvia);
                        } else if (data.sign == 'doublesub-cat') {
                            $('#category-select').empty();
                            $('#sub-category-select').empty();
                            $('#sub-sub-category-select').empty(); 
                            $('#'+data.id).empty().append(data.select_tag);
                            $('#'+data.idt).empty().append(data.select_tagvia);
                            $('#'+data.idt2).empty().append(data.select_tagvia2);
                        }   else {
                            $('#category-select').empty();
                            $('#sub-category-select').empty();
                            $('#sub-sub-category-select').empty();    
                    ...
                    } else if(type == 'no-select') {
                        $('#sub-category-select').empty();
                        
                        $('#sub-sub-category-select').empty();
                        $('#sub-category-select').append(data.select_tag); 
                    } else {
                        $('#sub-sub-category-select').empty();   
                        $('#sub-sub-category-select').append(data.select_tag);   
    ...
    <script>
        ...
        $(document).ready(function(){
           ...
           $(document).on('blur', 'input.product-title', function(){      
                ...
                $.ajax({
                    url: '{{ route('admin.product.category_by_product') }}',
                    method: "POST",                    
                    data: {query:query},
             ...
             $(document).on('click', 'label.cat-title-label', function(){
                let id = $(this).find('input.cat-title').val();
                let text = $(this).find('span').text();
                $(document).find('select[name="category_id"').val(id);
                getRequest('{{ url('/') }}/admin/product/get-categories?id='+id,'','select');                
            });
